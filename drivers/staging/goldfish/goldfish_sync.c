...
goldfish_sync_cmd_queue(struct goldfish_sync_state *sync_state,
			...
			u64 hostcmd_handle)
{
	unsigned int to_do_end = sync_state->to_do_end;
	...
	if (to_do_end >= GOLDFISH_SYNC_MAX_CMDS) {
		const unsigned int to_do_begin = sync_state->to_do_begin;
		const unsigned int to_do_size = to_do_end - to_do_begin;
...
{
	u32 result;
	unsigned int to_do_begin;
	unsigned int to_do_end;
	unsigned int to_do_size;
